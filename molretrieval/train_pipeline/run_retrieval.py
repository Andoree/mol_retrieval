...
def test_model(model, tokenizer, original_test_df, augmented_sets_dict, max_length, smiles_col,
    ...
    res_dict = {}
    if bart_flag:
        orig_res = save_last_hidden_state_bart(original_test_df, model, tokenizer, 'orig',
    ...
    for augmentation_name, augmented_test_df in augmented_sets_dict.items():
        if bart_flag:
            test = save_last_hidden_state_bart(augmented_test_df, model, tokenizer, augmentation_name,
                                                   smiles_col=smiles_col, max_length=max_length)
        else:
            try:
...
def main(args):
    ...
    retrieval_res_dict = test_model(model, tokenizer, orig_test_df, augm_name2df, smiles_col="smiles",
                                    max_length=max_length, bart_flag=bart_flag)
