...
def save_last_hidden_state_model(data, model, tokenizer, path_to_res, max_length: int, smiles_col: str):
    batch_size = 1
    ...
    for i in tqdm(range(0, len(data[smiles_col]), batch_size)):
        encoded_dict_t5condgen = tokenizer(list(data[smiles_col][i:i + batch_size]),
                                           return_tensors="pt", padding=False, max_length=max_length, truncation=True)
...
def save_last_hidden_state_encoder(data, model, tokenizer, path_to_res, max_length: int, smiles_col: str):
    batch_size = 1
...
def count_res(test_data_path, orig_data_path):
    ...
    [sum(create_dist([orig_data[i][u] for i in range(3300)], [test_data[i][u] for i in range(3300)])[-1]) for u in
     range(len(orig_data[0]))]
    return [sum(create_dist([orig_data[i][u] for i in range(3300)], [test_data[i][u] for i in range(3300)])[-1]) for u
            in range(len(orig_data[0]))]
